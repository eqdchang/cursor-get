    ...
    <div className="bw-scope">
      ...
      >
        <div className="max-w-[1280px] mx-auto px-5">
          ...
          {/* Main content row: mission + 3 link cols + Info/Follow Us combined */}
          <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr_1fr_1fr] gap-8 mb-10">
            {/* Mission block */}
            <div>
              <h5 className="text-white font-normal mb-3 text-2xl">Our mission</h5>
              <p className="text-white/80 text-sm leading-relaxed mb-4">
                Enabling better, smarter, safer healthcare to improve lives
              </p>
              <a
                href="/en-us/home/our-company/"
                className="inline-block text-[#05DD4D] border-2 border-[#05DD4D] rounded-full px-4 py-[9px] text-base font-bold hover:bg-[#05DD4D] hover:text-[#01332B] transition-colors"
              >
                Learn more
              </a>
            </div>

            {/* Our company + Resources & education columns */}
            {COLUMNS.filter((c) => c.heading !== "Info").map((col) => (
              <div key={col.heading}>
                <h5 className="text-white font-normal mb-3 text-2xl">{col.heading}</h5>
                  ...
                  {col.links.map((link) => (
                    <li key={link.label}>
                      <a
                        ...
                        className="text-[#05DD4D] text-base underline hover:no-underline"
                      >
                        {link.label}
                      </a>
                    </li>
            ...
            {/* Info + Follow Us — stacked in the same column */}
            <div>
              <h5 className="text-white font-normal mb-3 text-2xl">Info</h5>
              <ul className="space-y-2 mb-8">
                {COLUMNS.find((c) => c.heading === "Info")!.links.map((link) => (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      {...(link.external || ext(link.href) ? { target: "_blank", rel: "noopener noreferrer" } : {})}
              ...
              <h5 className="text-white font-normal mb-3 text-2xl">Follow Us</h5>
              <div className="flex gap-3">
                {SOCIAL_LINKS.map((s) => (
                  <a
                    ...
                    aria-label={s.label}
                    className="flex items-center justify-center w-8 h-8 rounded-full border-2 border-[#05DD4D] text-[#05DD4D] transition-colors shrink-0"
                  >
                    {s.icon}
                  </a>
                ))}
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1">
              {LEGAL_LINKS.slice(0, -1).map((link) => (
                <a
                  ...
                  href={link.href}
                  {...(link.external || ext(link.href) ? { target: "_blank", rel: "noopener noreferrer" } : {})}
                  className="text-[#05DD4D] text-xs font-bold underline hover:no-underline"
                >
                  {link.label}
                </a>
              ))}
